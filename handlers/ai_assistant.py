...
ВАЖНО: Когда пользователь спрашивает про текущую погоду, актуальные события, новости или любую информацию требующую актуальных данных — ВСЕГДА используй инструмент web_search для получения свежей информации. Не отвечай из памяти на вопросы про "сейчас", "сегодня", "текущий".
...
TOOLS = [
    {
        ...
        "function": {
            "name": "web_search",
            "description": "Поиск актуальной информации в интернете. Используй для получения текущей погоды, новостей, актуальных событий и любой информации которая может измениться.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Поисковый запрос на английском языке для лучших результатов"
                    }
                },
                "required": ["query"]
            }
