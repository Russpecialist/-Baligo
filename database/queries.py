...
async def get_user_restaurants(chat_id: int) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения ресторанов пользователя {chat_id}: {e}")
        return []
...
async def add_user_restaurant(chat_id: int, restaurant_id: int) -> bool:
    ...
    try:
        ...
        if existing:
            ...
            logger.info(f"Связь пользователя {chat_id} с рестораном {restaurant_id} уже существует")
            return True
        else:
            ...
            logger.info(f"Связь пользователя {chat_id} с рестораном {restaurant_id} добавлена")
            return True
...
async def remove_user_restaurant(chat_id: int, restaurant_id: int) -> bool:
    ...
    try:
        ...
        await db_manager.execute_query(query, chat_id, restaurant_id)
        logger.info(f"Связь пользователя {chat_id} с рестораном {restaurant_id} удалена")
        return True
...
async def get_restaurant_moderators(restaurant_id: int) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения модераторов ресторана {restaurant_id}: {e}")
        return []
...
async def get_restaurant_id_by_name(restaurant_name: str) -> Optional[int]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения restaurant_id для {restaurant_name}: {e}")
        return None
    ...
    try:
        ...
        if result:
            ...
            if approval_dict.get('created_at') is None:
                logger.warning(f"created_at is None for approval_id={approval_id}")
...
async def approve_request(approval_id: int, admin_chat_id: int) -> bool:
    ...
    try:
        ...
        async with pool.acquire() as connection:
            ...
            if result == 'UPDATE 1':
                logger.info(f"Запрос {approval_id} подтвержден админом {admin_chat_id}")
                # Применяем изменения
...
async def reject_request(approval_id: int, admin_chat_id: int) -> bool:
    ...
    try:
        ...
        async with pool.acquire() as connection:
            ...
            if result == 'UPDATE 1':
                logger.info(f"Запрос {approval_id} отклонен админом {admin_chat_id}")
                return True
            return False
...
async def apply_approval(approval_id: int) -> bool:
    ...
    try:
        ...
        if field_name == 'menu':
            ...
            file_name = os.path.basename(new_value) if os.path.exists(new_value) else new_value
            query = '''
                UPDATE restaurant
                SET menu = $1
            ...
            logger.info(f"Изменения применены к ресторану {restaurant_id}, поле {field_name}")
            return True
        ...
        if field_name == 'banquet':
            logger.info(f"Изменения банкета применены к ресторану {restaurant_id} (файл уже перемещен)")
            return True
...
async def create_restaurant(
    ...
    try:
        ...
        async with pool.acquire() as connection:
            ...
            if result:
                logger.info(f"Создан ресторан {restaurant_id}: {restaurant_name}")
                return result['restaurant_id']
            return None
...
async def update_restaurant(restaurant_id: int, **fields) -> bool:
    ...
    try:
        ...
        allowed_fields = ['restaurant_name', 'address_nm', 'reservation', 'delivery', 'region_nm', 'menu', 'promo', 'banquet_flg']
        ...
        logger.info(f"Ресторан {restaurant_id} обновлен: {', '.join(fields.keys())}")
        return True
...
async def get_promotions(restaurant_id: int, status: str = 'approved') -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения акций для ресторана {restaurant_id}: {e}")
        return []
...
async def get_events(restaurant_id: int, status: str = 'approved') -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения событий для ресторана {restaurant_id}: {e}")
        return []
...
async def get_promotion_event_approval(approval_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка получения запроса на согласование {approval_id}: {e}")
        return None
